...
def set_background(png_file):
    try:
        ...
        .stApp {{
            background-image: linear-gradient(rgba(11, 15, 25, 0.85), rgba(11, 15, 25, 0.85)), url("data:image/jpg;base64,{bin_str}");
...
if uploaded_file is not None:
    ...
    # --- TAB 1: DESCRIPTIVE FILTER (Your existing code) ---
    with tab1:
        ...
        if st.sidebar.button("Generate Report", type="primary"):
            with st.spinner("Crunching the data..."):
                ...
                if filtered_data.empty:
                    ...
                    final_result = top_campaigns[existing_columns[:28]] # Limiting to first 28 cols for display
    ...
    with tab2:
        ...
        if not results_col:
            ...
            if len(train_data) < 10:
                ...
                max_sim_spend = float(X[amount_spent_col].max() * 2) # Allow simulating up to 2x max historical spend
                ...
                with col2:
                    st.warning(f"**Estimated Cost per Result:**\n### ₹{est_cpa:.2f}") # Assuming INR, change to $ if needed
